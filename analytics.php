...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($pdo) {
        ...
        $completed_work = '';
        $stmt = $pdo->prepare("INSERT INTO analytics_requests (user_id, state, institution, department, program_type, country, software, project_topic, has_topic, chapter3_file, payment_receipt, payment_amount, currency, order_id, completed_work) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$user_id, $state, $institution, $department, $program_type, $country, $software, $project_topic, $has_topic, $chapter3_file, $payment_receipt, $payment_amount, $currency, $order_id, $completed_work]);
    }
